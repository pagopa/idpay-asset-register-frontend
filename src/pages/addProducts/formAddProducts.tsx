...
const FormAddProducts = forwardRef<FormAddProductsRef, Props>(
  ...
  ({ fileAccepted, setFileAccepted }, ref) => {
    ...
    const processFileUpload = async (files: Array<File>) => {
      ...
      try {
        const res = await uploadProductListVerify(files[0], formik.values.category as UploadProductListParams["category"]);
    ...
    const uploadFileAndNavigate = async () => {
      ...
      const res = await uploadProductList(fileState.currentFile, formik.values.category as UploadProductListParams["category"]);
