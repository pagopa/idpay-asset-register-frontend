...
describe('UploadsTable', () => {
        ...
        content: [
            {
                ...
                batchName: 'Batch A',
                dateUpload: new Date('2025-07-21T10:00:00Z'),
    ...
    it('renders loading state', () => {
        renderComponent({ loading: true, error: null, data: null, page: 0, rowsPerPage: 10, totalElements: 0, onPageChange: jest.fn(), onRowsPerPageChange: jest.fn() });
        expect(screen.getByRole('progressbar')).toBeInTheDocument();
    });

    it('renders error state', () => {
        renderComponent({ loading: false, error: 'Errore', data: null, page: 0, rowsPerPage: 10, totalElements: 0, onPageChange: jest.fn(), onRowsPerPageChange: jest.fn() });
        expect(screen.getByText('Errore')).toBeInTheDocument();
    });

    it('renders table with data', () => {
        renderComponent({ loading: false, error: null, data: mockData, page: 0, rowsPerPage: 10, totalElements: 1, onPageChange: jest.fn(), onRowsPerPageChange: jest.fn() });
        expect(screen.getByTestId('uploads-table')).toBeInTheDocument();
        expect(screen.getByText('Batch A')).toBeInTheDocument();
        expect(screen.getByText('5 pages.uploadHistory.uploadHistoryFoundProducts')).toBeInTheDocument();
        expect(screen.getByText('3 pages.uploadHistory.uploadHistoryAddedProducts')).toBeInTheDocument();
    });
    ...
    it('renders empty table message', () => {
        renderComponent({ loading: false, error: null, data: { content: [] }, page: 0, rowsPerPage: 10, totalElements: 0, onPageChange: jest.fn(), onRowsPerPageChange: jest.fn() });
