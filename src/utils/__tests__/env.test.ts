...
jest.mock('env-var', () => ({
  get: jest.fn((key: string) => {
    const mockValues: Record<string, any> = {
      ...
      REACT_APP_URL_FE_ASSISTANCE_ASSET_REGISTER: {
        required: () => ({ asString: () => '/assistance' }),
      },
      REACT_APP_URL_FE_EIE_MANUAL: { asString: () => '/manual' },
      REACT_APP_URL_API_REGISTER: {
        required: () => ({ asString: () => 'https://mock-api/register' }),
      },
      ...
      REACT_APP_MIXPANEL_API_HOST: {
        default: () => ({ asString: () => 'https://api-eu.mixpanel.com' }),
      },
      REACT_APP_ONE_TRUST_OTNOTICE_CDN_URL: { required: () => ({ asString: () => 'cdn-url' }) },
      REACT_APP_ONE_TRUST_OTNOTICE_CDN_SETTINGS: {
        required: () => ({ asString: () => 'cdn-settings' }),
      },
      REACT_APP_ONE_TRUST_PRIVACY_POLICY_ID: { required: () => ({ asString: () => 'privacy-id' }) },
      REACT_APP_ONE_TRUST_PRIVACY_POLICY_JSON_URL: {
        required: () => ({ asString: () => 'privacy-json' }),
      },
      REACT_APP_ONE_TRUST_TOS_ID: { required: () => ({ asString: () => 'tos-id' }) },
      REACT_APP_ONE_TRUST_TOS_JSON_URL: { required: () => ({ asString: () => 'tos-json' }) },
