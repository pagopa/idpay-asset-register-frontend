...
const Layout = ({ children }: Props) => {
  ...
  const match = SIDEMENU_MATCH_PATHS.some(
    (path) => matchPath({ path, end: path === ROUTES.HOME }, location.pathname) !== null
  );

  const showAssistanceInfo = useMemo(() => {
    const assistanceMatch = matchPath({ path: ROUTES.ASSISTANCE, end: false }, location.pathname);
    return assistanceMatch === null;
  }, [location.pathname]);
